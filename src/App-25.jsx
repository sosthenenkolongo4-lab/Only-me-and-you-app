...
import {
  ...
  MessageSquareText, Library, Search, Mail, Pencil
} from "lucide-react";
...
function Chat({ data, name, save }) {
  ...
  const sendCard = () => {
    ...
    save({
      ...
      messages: [...data.messages, {
        ...
        type: "card", cardText: cardText.trim(), theme: cardTheme, openedBy: []
      }]
    });
    setCardText("");
    setShowCard(false);
  ...
  const openCard = (m) => {
    ...
    if (!(m.openedBy || []).includes(name)) {
      save({
        ...data,
        messages: data.messages.map(x => x.id === m.id ? { ...x, openedBy: [...(x.openedBy || []), name] } : x)
      });
    }
  };
  ...
  const closeReveal = () => {
    if (revealCard) {
      // La carte redevient scellée pour la personne qui la referme, mais reste dans le chat,
      // rouvrable à tout moment par n'importe qui.
      save({
        ...data,
        messages: data.messages.map(x => x.id === revealCard.id ? { ...x, openedBy: (x.openedBy || []).filter(n => n !== name) } : x)
      });
    }
    setRevealCard(null);
  };
    ...
    <div>
      ...
      <div className="chat card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
        ...
        {visibleMessages.map(m => {
          ...
          const isSealed = m.type === "card" && !(m.openedBy || []).includes(name);

          return (
            <div
              className={"msg " + (mine ? "mine" : "theirs")}
              key={m.id}
              onClick={e => isSealed ? openCard(m) : openMenu(e, m)}
            ...
            >
              ...
              <small>
                {m.edited && !isSealed && m.type !== "card" && <span style={{ opacity: 0.7, marginRight: '4px' }}>Modifié ·</span>}
      ...
      {mode === "question" && (() => {
        ...
        return (
          <div className="game card">
            ...
            <button className="secondary" onClick={() => setMode("questionHistory")}><BookOpenText size={15} /> Voir les questions précédentes</button>
            <button className="secondary" onClick={() => setMode("menu")}>← Retour</button>
          </div>
        );
      })()}

      {mode === "questionHistory" && (() => {
        const todayQ = getDailyQuestion(getDayKey());
        const seen = new Set();
        const entries = [];
        data.gameAnswers
          .filter(a => questions.includes(a.prompt) && a.prompt !== todayQ)
          .sort((x, y) => new Date(y.date) - new Date(x.date))
          .forEach(a => {
            if (seen.has(a.prompt)) return;
            seen.add(a.prompt);
            const mine = data.gameAnswers.find(x => x.prompt === a.prompt && isMine(x));
            const other = data.gameAnswers.find(x => x.prompt === a.prompt && !isMine(x));
            entries.push({ prompt: a.prompt, mine, other });
          });
        return (
          <div className="game card">
            <BookOpenText size={22} />
            <div className="tag">QUESTIONS PRÉCÉDENTES</div>
            {entries.length === 0 && <p style={{ fontSize: '0.85rem', color: '#8A5568' }}>Rien à revoir pour l'instant — revenez demain !</p>}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%', maxHeight: '380px', overflowY: 'auto' }}>
              {entries.map(e => (
                <div key={e.prompt} style={{ background: '#fff', border: '1px solid #f1dbe4', borderRadius: '12px', padding: '10px 12px', textAlign: 'left' }}>
                  <div style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: '6px' }}>{e.prompt}</div>
                  {e.mine && <div style={{ fontSize: '0.8rem', marginBottom: '4px' }}><b>Toi :</b> {e.mine.answer}</div>}
                  {e.other
                    ? <div style={{ fontSize: '0.8rem' }}><b>{partnerName} :</b> {e.other.answer}</div>
                    : <div style={{ fontSize: '0.8rem', opacity: 0.6 }}><b>{partnerName} :</b> pas encore répondu</div>}
                </div>
              ))}
            </div>
            <button className="secondary" onClick={() => setMode("question")}>← Retour à aujourd'hui</button>
          </div>
        );
      })()}
      {mode === "truth" && (
