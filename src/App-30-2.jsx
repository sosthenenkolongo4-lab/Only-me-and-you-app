...
// --- Données "Découvrir" : activités par sujet + quiz du langage de l'amour ---
...
function Games({ data, save, name, role, onOpenPacks, jumpTo, onConsumeJump }) {
  ...
  const saveAnswer = (prompt, text, topic) => {
    if (!text || !text.trim()) return;
    save({ ...data, gameAnswers: [{ id: uid(), prompt, answer: text, topic, date: new Date().toISOString(), author: name, authorRole: role, comments: [] }, ...data.gameAnswers] });
    setAnswer("");
    setMode(topic ? "topic" : "hub");
  };
  ...
  const titleFor = () => {
    ...
    if (mode === "menu" || ["truth", "wyr", "wheel", "outings"].includes(mode)) return { title: "Jeux à deux", sub: "Riez, parlez et découvrez-vous encore." };
  ...
  return (
    <div>
        ...
        return (
          <div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {t.prompts.map(p => {
                const answered = data.gameAnswers.some(a => a.prompt === p && a.topic === t.key && isMine(a));
                return (
      ...
      {mode === "topic" && activePrompt && (() => {
        const t = discoverTopics.find(t => t.key === activeTopic);
        const myAnswer = data.gameAnswers.find(a => a.prompt === activePrompt && a.topic === activeTopic && isMine(a));
        const partnerAnswer = data.gameAnswers.find(a => a.prompt === activePrompt && a.topic === activeTopic && !isMine(a));
        return (
          <div className="game card">
