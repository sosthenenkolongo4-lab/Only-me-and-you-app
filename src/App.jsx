      ...
      <div className="chat card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch' }}>
        ...
        {data.messages.map(m => {
          ...
          const bubbleBase = {
            maxWidth: '82%',
            width: 'fit-content',
            padding: m.type === 'card' ? 0 : '12px 16px',
            borderRadius: mine ? '20px 20px 6px 20px' : '20px 20px 20px 6px',
            background: m.deleted ? '#f3c9d4' : m.type === 'card' ? 'transparent' : mine ? '#C9184A' : '#ffffff',
            color: m.deleted ? '#7a3b4c' : mine ? '#ffffff' : '#2b1018',
            boxShadow: m.type === 'card' ? 'none' : '0 3px 10px rgba(0,0,0,0.06)',
            display: 'flex',
          ...
          return (
            <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: mine ? 'flex-end' : 'flex-start', width: '100%' }}>
              <div
                onClick={e => openMenu(e, m)}
                style={bubbleBase}
              >
                {m.deleted ? (
                  <span style={{ fontStyle: 'italic', opacity: 0.7, display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                  ...
                  <span style={{ fontSize: '0.96rem' }}>{m.text}</span>
                )}
                <small style={{ fontSize: '0.68rem', opacity: mine ? 0.8 : 0.5, alignSelf: 'flex-end' }}>
