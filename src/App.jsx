...
import {
  ...
  MessageSquareText, Library, Search, Mail
} from "lucide-react";
    ...
    <div className="home">
      ...
      <div className="today card">
        ...
        {(() => {
          ...
          const myAnswer = data.gameAnswers.find(a => a.prompt === todayQ && a.author === name);
          const partnerAnswer = data.gameAnswers.find(a => a.prompt === todayQ && a.author !== name);
          if (myAnswer) {
            return (
              <div style={{ fontSize: '0.85rem', marginTop: '4px' }}>
                <div style={{ opacity: 0.75 }}>Ta réponse : {myAnswer.answer}</div>
                <div style={{ opacity: 0.75, marginTop: '2px' }}>
                  {partnerName} : {partnerAnswer ? partnerAnswer.answer : "pas encore répondu"}
                </div>
              </div>
            );
          }
          return (
            <>
              <textarea
                ...
                style={{ width: '100%', minHeight: '60px', marginTop: '6px', boxSizing: 'border-box' }}
              />
              <button
                onClick={() => {
                  if (!dailyAnswer.trim()) return;
                  save({ ...data, gameAnswers: [{ id: uid(), prompt: todayQ, answer: dailyAnswer.trim(), date: new Date().toISOString(), author: name, comments: [] }, ...data.gameAnswers] });
                  setDailyAnswer("");
                }}
              >
                Garder notre réponse <ArrowRight size={14} />
              </button>
            </>
          );
        })()}
...
function Chat({ data, name, save }) {
  ...
  const sendCard = () => {
    ...
    save({
      ...
      messages: [...data.messages, {
        ...
        type: "card", cardText: cardText.trim(), theme: cardTheme, openedBy: [name]
      }]
    });
    setCardText("");
    setShowCard(false);
  ...
  const closeReveal = () => {
    if (revealCard) {
      save({
        ...data,
        messages: data.messages.map(x => x.id === revealCard.id ? { ...x, hiddenFor: [...(x.hiddenFor || []), name] } : x)
      });
    }
    setRevealCard(null);
  };
  ...
  return (
    <div>
      ...
      {mode === "question" && (() => {
        ...
        const myAnswer = data.gameAnswers.find(a => a.prompt === q && a.author === name);
        const partnerAnswer = data.gameAnswers.find(a => a.prompt === q && a.author !== name);
        return (
          <div className="game card">
            ...
            {myAnswer ? (
              <div style={{ fontSize: '0.85rem', margin: '8px 0' }}>
                <div>Ta réponse : {myAnswer.answer}</div>
                <div style={{ marginTop: '4px', opacity: 0.75 }}>
                  Ton/ta partenaire : {partnerAnswer ? partnerAnswer.answer : "pas encore répondu"}
                </div>
              </div>
            ) : (
              <>
